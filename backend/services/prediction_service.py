...
def _to_dict(data):
    ...
    return {feature: getattr(data, feature) for feature in feature_names}
...
def predict(data):
    ...
    df = pd.DataFrame([{feature: data[feature] for feature in feature_names}])
    df = df.apply(pd.to_numeric, errors="coerce")

    invalid_features = df.columns[df.isna().any()].tolist()
    if invalid_features:
        raise ValueError(f"Invalid numeric values for: {', '.join(invalid_features)}")
    ...
    return {
        ...
        "risk_level": "High" if int(prediction) else "Low",
        "status": "Stressed" if int(prediction) else "Healthy",
    }
